...
worth\tsh and to wear a mask which was bald,\catch{or} or which
...
us to sup-\catch{pose,} pose, but that my uncle Toby’s
...
one would think I took a pleasure in run-\catch{ing} ing into
...
two,\catch{till} till corporal Trim (who to the character of an
...
nothing)\tsh so much the worse\catch{for} for the public; they were two Flemish ells
...
here I break my meta-\break phor)\tsh\patch{Crust}
...
much alike both upon it and other matters)\tsh\catch{\textsc{Love}} \textsc{Love} is
...
garden which join’d and\catch{laid} laid parallel to Mrs.\@ Wadman’s; this, with the advantage of a thickset
...
hand as soon as she possibly could;\catch{which,} which, under one pretence or
...
\lettrine{T}{\,\ls{hese}} attacks of Mrs.\@ Wadman,
...
sound more sorrowfully to his ear, forbad him.\patch{\tsk No;}
...
which, from what thou hast dropt, I\catch{partly} partly suspect to be the
...
mountains it takes its\catch{rise} rise\tsk what is its course\tsk how far
...
could have happen’d no way.\patch{The}
...
which covered the re-\catch{treat} treat over the bridge Neerspeeken, the king himself could
...
Toby\tsk he deserves\catch{a} a crown \tsh As richly, as a
...
room\tsk and die,\catch{than} than go on\tsh when, upon her
...
to say,\break\lqq God bless me.”\patch{I thought}
...
floating in it\catch{\tsk There} \tsk there is nothing, my dear paternal uncle! but
...
Toby and the corporal\break went on with theirs.\etp\patch{Now}
...
extract so much\tsh\etp\patch{I wish,}
...
being now close beside me, preparing for bed\tsh\catch{I have}
...
are\tsh else there would be no dealing with them.\patch{Let}
...
thin scarlet\catch{breeches} breeches was laid aside (at least for the present),
...
letter of in-\catch{structions} structions into my uncle Toby’s coat-pocket\tsh and join with my mother
